...
提出了平衡的有监督对比学习方法，网络包含了分类分支和对比学习分支，最后使用Logit Compensation优化输出。对同一张图片做三种不同的数据增强，同时输入模型，第一张得到使用logit compensation修改的CE loss，并对第一张的logit非线性转换作为Prototype加入到和二三张图片的对比损失中。
